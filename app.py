...
def show_leaderboard():
    ...
    my_ps=users.get(st.session_state.user_email,{}).get("predictions",[]); my_b=max([p["salary"] for p in my_ps],default=0)
    if my_b:
        my_rank=sum(1 for e in lb if e["salary"]>my_b)+1
        st.markdown(f'<div style="background:linear-gradient(135deg,#eef2ff,#f5f3ff);border:1px solid #c7d2fe;border-radius:14px;padding:18px 24px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;"><div><div style="font-size:11px;color:#6366f1;font-weight:700;text-transform:uppercase;">Your Position</div><div style="font-size:22px;font-weight:800;color:#1e1b4b;font-family:\'Syne\',sans-serif;">#{my_rank} <span style="font-size:14px;color:#64748b;font-weight:400;">out of {len(lb)} users</span></div></div><div style="text-align:right;"><div style="font-size:11px;color:#6366f1;font-weight:700;text-transform:uppercase;">Your Best</div><div style="font-size:22px;font-weight:800;color:#4f46e5;font-family:\'Syne\',sans-serif;">₹{my_b:,}</div></div></div>', unsafe_allow_html=True)
    if not lb:
        st.markdown('<div style="text-align:center;padding:40px;color:#94a3b8;">No predictions yet. Be the first!</div>', unsafe_allow_html=True)
    else:
        medals=["🥇","🥈","🥉"]; rcs=["gold","silver","bronze"]
        st.markdown('<div class="card"><div class="card-title">🏆 Top Earners</div>', unsafe_allow_html=True)
        for i,entry in enumerate(lb):
            m=medals[i] if i<3 else f"#{i+1}"; rc=rcs[i] if i<3 else ""; im=entry["name"]==st.session_state.user_name
            yb='&nbsp;<span style="font-size:11px;background:#eef2ff;color:#4f46e5;padding:2px 8px;border-radius:99px;font-weight:600;">You</span>' if im else ""
            st.markdown(f'<div class="lb-row {rc}" style="{"border:2px solid #6366f1;" if im else ""}"><div class="lb-rank">{m}</div><div style="flex:1;"><div class="lb-name">{entry["name"]}{yb}</div><div class="lb-role">{entry["job"]} &nbsp;·&nbsp; {entry["exp"]} yrs</div></div><div class="lb-salary">₹{entry["salary"]:,}</div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
